  ...
  return (
    <>
      ...
      <section className="block"><div className="wrap">
        ...
        <div className="cols2">
          <div><div className="icrow"><I n="check" /><h4 style={{ margin: 0, color: 'var(--accent)' }}>Real</h4></div><p style={{ color: 'var(--dim)', marginBottom: 0 }}>Two <b>separate Band organizations</b> (@northwind-analytics and @lumen-retail) with a genuine cross-org boundary — the owner must accept the contact. Two independent operators run it out of the box (<code>run_org.py buyer|owner</code> + <code>JOIN.md</code>), each with only their org’s creds. Consent, counter-offer, refusal and the human gate are genuine LLM/Band events; the export gate, k-anonymity, differential privacy and Ed25519 signing are deterministic code under <b>124 tests</b>. Cross-vendor demonstrated live on Groq and OpenRouter.</p></div>
          <div><div className="icrow"><I n="ban" /><h4 style={{ margin: 0, color: 'var(--bad)' }}>Demo-scoped</h4></div><p style={{ color: 'var(--dim)', marginBottom: 0 }}>The owner’s dataset is synthetic (the clean room aggregates real generated rows). The <i>recorded</i> walkthrough was driven by one operator holding both org accounts — the code runs two real operators unchanged. Band’s Memory API is Enterprise-gated, so cross-deal persistence falls back to a local store.</p></div>
        </div>
      </div></section>
